...
1. **npm scope.** `@usebones` has zero published packages and matches the
   domain — recommended. Scope/org availability can only be confirmed while
   logged in to npm (create the org at npmjs.com → Add Organization). The
   bare `bones` package name is taken by an unmaintained package, and public
   org pages can't be probed anonymously. Action: create the `usebones` org
   (and grab `bones-ui` defensively if free).
2. **Commercial-tier location.** Options: (a) source-available folder in this
...
3. **Commercial-tier distribution.** Options: authenticated component
...
4. **Docs engine.** Plain MDX in Next.js vs a docs framework with built-in
   search + llms.txt generation. Decide when docs content starts for real.
5. **Per-component packages?** `@usebones/button`-style installs are possible
...
- **Foundation:** Base UI (`@base-ui/react` — note the 2026 rename from
