...
| `apps/www`        | —                 | Landing page + docs (Next.js).                      | —          |
| `interface/`      | —                 | Commercial tier (AI chat, chat bubbles, thinking states). Reserved, see its README. | Commercial |
...
- Variants/sizes via data attributes (`data-variant`, `data-size`), styled in
  CSS — not class permutations, not inline styles.
...
- Docs and code comments: lowercase "bones", plain language, no hype.
...
- Adding a runtime dependency to any `packages/*` package is a significant
  decision — ask first. Dev dependencies are fine.
